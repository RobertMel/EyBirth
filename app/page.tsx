...
import { LeafSpray } from "@/components/SavannaMotifs"
...
export default function Page() {
  ...
  return <main className="min-h-screen overflow-hidden bg-[#f5eddf] px-4 py-8 text-[#536755] sm:px-6"><div className="mx-auto max-w-xl"><div className="mb-7 flex items-center justify-between px-2"><span className="text-[10px] uppercase tracking-[0.28em] text-[#9a927f]">Invitation privée</span><span className="flex items-center gap-1.5 text-[10px] uppercase tracking-[0.18em] text-[#536755]"><CalendarDays size={13} /> 21.11.26</span></div><section className="relative overflow-hidden rounded-[2rem] border border-[#536755]/45 bg-[#fffaf1] px-6 py-10 text-center shadow-[0_18px_55px_rgba(83,103,85,0.14)] sm:px-12"><span className="corner-ribbon corner-ribbon-left" aria-hidden="true" /><span className="corner-ribbon corner-ribbon-right" aria-hidden="true" /><div className="pointer-events-none absolute inset-3 rounded-[1.6rem] border border-[#536755]/20" /><div aria-hidden="true" className="pointer-events-none absolute inset-0 z-0 overflow-hidden">
      <div className="absolute -left-2 -top-2 h-24 w-24 opacity-70"><LeafSpray className="absolute inset-0 h-full w-full" /><img src="/animals/lion.png" alt="" className="absolute left-6 top-6 h-11 w-11 object-contain drop-shadow-sm" /></div>
      <div className="absolute -right-2 -top-2 h-24 w-24 -scale-x-100 opacity-70"><LeafSpray className="absolute inset-0 h-full w-full" /></div>
      <img src="/animals/gorilla.png" alt="" className="absolute right-3 top-7 h-10 w-10 object-contain opacity-90 drop-shadow-sm" />
      <img src="/animals/giraffe.png" alt="" className="absolute left-1 top-32 h-12 w-12 object-contain opacity-85 drop-shadow-sm" />
      <img src="/animals/zebra.png" alt="" className="absolute right-1 top-36 h-10 w-10 -scale-x-100 object-contain opacity-85 drop-shadow-sm" />
      <img src="/animals/elephant.png" alt="" className="absolute left-2 top-64 h-10 w-10 object-contain opacity-85 drop-shadow-sm" />
      <span className="absolute right-2 top-64 text-xl opacity-45" aria-hidden="true">🐆</span>
      <img src="/animals/giraffe.png" alt="" className="absolute left-1 top-96 h-11 w-11 -scale-x-100 object-contain opacity-70 drop-shadow-sm" />
      <img src="/animals/gorilla.png" alt="" className="absolute right-1 top-96 h-9 w-9 object-contain opacity-70 drop-shadow-sm" />
      <img src="/animals/zebra.png" alt="" className="absolute left-2 top-[46%] h-9 w-9 object-contain opacity-60 drop-shadow-sm" />
      <img src="/animals/lion.png" alt="" className="absolute right-2 top-[46%] h-9 w-9 -scale-x-100 object-contain opacity-60 drop-shadow-sm" />
      <span className="absolute left-3 top-[68%] text-lg opacity-35" aria-hidden="true">🐆</span>
      <img src="/animals/elephant.png" alt="" className="absolute right-3 top-[68%] h-8 w-8 -scale-x-100 object-contain opacity-55 drop-shadow-sm" />
      <img src="/animals/giraffe.png" alt="" className="absolute left-4 bottom-6 h-9 w-9 object-contain opacity-55 drop-shadow-sm" />
      <img src="/animals/zebra.png" alt="" className="absolute right-4 bottom-7 h-8 w-8 -scale-x-100 object-contain opacity-55 drop-shadow-sm" />
      <img src="/animals/lion.png" alt="" className="absolute left-3 bottom-2 h-9 w-9 object-contain opacity-60 drop-shadow-sm" />
      <img src="/animals/gorilla.png" alt="" className="absolute right-3 bottom-1 h-8 w-8 -scale-x-100 object-contain opacity-60 drop-shadow-sm" />
      <div className="absolute -left-2 -bottom-2 h-20 w-20 rotate-90 opacity-45"><LeafSpray className="absolute inset-0 h-full w-full" /></div>
      <div className="absolute -right-2 -bottom-2 h-20 w-20 -rotate-90 opacity-45"><LeafSpray className="absolute inset-0 h-full w-full" /></div>
    </div><div className="relative z-10"><div className="mx-auto mb-7 flex h-11 w-11 items-center justify-center rounded-full bg-[#dce5d7] text-[#788b76]"><Gift size={20} strokeWidth={1.4} /></div><p className="text-[10px] uppercase tracking-[0.4em] text-[#536755]">Une première bougie, un grand bonheur à partager...</p><h1 className="name-idle relative mt-4 select-none font-serif text-6xl italic tracking-tight text-[#536755] sm:text-7xl">Eyalane<span key={nameBurst} aria-hidden="true" className="pointer-events-none absolute inset-0">{["🦁", "🦒", "🐘", "🦓", "🦍", "🐆"].map((e, i) => <span key={i} className="name-burst-emoji" style={{ "--i": i } as any}>{e}</span>)}</span></h1><div className="mx-auto my-5 w-64 sm:w-72"><img src="/photos/eyalane-torn.png" alt="Eyalane" className="h-auto w-full drop-shadow-[0_12px_28px_rgba(83,103,85,0.28)]" /></div><p className="font-serif text-3xl text-[#536755]">1 an</p><p className="mx-auto mt-5 max-w-xs text-sm leading-6 text-[#7d8273]">Venez célébrer ce merveilleux moment à nos côtés et créer avec nous de précieux souvenirs ! 🥳</p><div className="mt-7 flex items-start justify-center divide-x divide-[#536755]/30 text-[10px] text-[#687166]"><div className="flex min-w-0 flex-1 flex-col items-center gap-2 px-2"><CalendarDays size={15} className="text-[#536755]" /><span>21 novembre</span></div><div className="flex min-w-0 flex-1 flex-col items-center gap-2 px-2"><Clock3 size={15} className="text-[#536755]" /><span>13h00</span></div><div className="flex min-w-0 flex-1 flex-col items-center gap-2 px-2"><MapPin size={15} className="text-[#536755]" /><span>Viry Châtillon</span></div></div><div className="mx-auto mt-6 flex w-fit items-center gap-3 rounded-full border border-[#536755]/25 bg-[#fbf6eb] px-4 py-2 text-[10px] tracking-wide text-[#78806f]"><span className="font-medium text-[#536755]">Dress code</span><span className="flex items-center gap-1.5"><span className="h-2.5 w-2.5 rounded-full border border-[#4a5a4c]/20 bg-[#536755]" />Vert sauge</span><span className="flex items-center gap-1.5"><span className="h-2.5 w-2.5 rounded-full border border-[#d9cdb8] bg-[#ece2cf]" />Beige</span></div><Countdown /><RsvpForm /></div></section><button onClick={() => setAdmin(true)} aria-label="Ouvrir la vue admin" className="fixed bottom-4 right-4 rounded-full border border-[#536755]/30 bg-[#fffaf1]/80 p-3 text-[#9b8764] shadow-sm backdrop-blur transition hover:bg-white"><LockKeyhole size={14} /></button><p className="py-7 text-center text-[10px] uppercase tracking-[0.28em] text-[#aaa18f]">Avec tout notre amour</p></div></main>
}
