...
function Countdown() {
  ...
  return <div className="mt-7 grid grid-cols-4 gap-2" aria-label="Compte à rebours">{values.map(([label, value]) => <div key={label} className="rounded-2xl border border-[#8b5e3c]/25 bg-[#fbf6eb] px-1 py-3 text-center"><div className="font-serif text-xl text-[#8b5e3c]">{String(value).padStart(2, "0")}</div><div className="mt-1 text-[8px] uppercase tracking-[0.12em] text-[#8d8b78]">{label}</div></div>)}</div>
}

function Confetti() { return <div className="pointer-events-none fixed inset-0 z-50 overflow-hidden" aria-hidden="true">{Array.from({ length: 24 }, (_, i) => <span key={i} className="confetti" style={{ left: `${(i * 37) % 100}%`, animationDelay: `${(i % 8) * 70}ms`, backgroundColor: i % 3 === 0 ? "#8b5e3c" : i % 3 === 1 ? "#9dad96" : "#b8875c" }} />)}</div> }
...
function RsvpForm() {
  ...
  if (submitted) return <div className="mt-8 rounded-3xl border border-[#8b5e3c]/25 bg-[#f8f0df] p-6 text-center" role="status"><div className={`mx-auto mb-3 flex h-10 w-10 items-center justify-center rounded-full text-white ${attending ? "bg-[#a8b7a0]" : "bg-[#c99d87]"}`}>{attending ? <Check size={18} /> : <X size={18} />}</div><h3 className="font-serif text-2xl text-[#536755]">{attending ? "Quelle joie !" : "Oh, quel dommage..."}</h3><p className="mt-2 text-sm leading-6 text-[#727463]">{attending ? <>Merci pour votre réponse. Nous avons hâte de vous retrouver !<br /><strong className="font-medium text-[#536755]">123 Rue de la Fête, 91390 Viry Châtillon</strong></> : "On pensera fort à vous pour cette belle journée."}</p>{attending && <p className="mt-4 text-xs italic text-[#8b806e]">Votre présence est déjà le plus beau des cadeaux — mais si le cœur vous en dit, Eyalane serait ravi d'une petite surprise 🎁</p>}</div>

  return <form onSubmit={submit} className="mt-8 border-t border-[#8b5e3c]/20 pt-8 text-left">
    <div className="mb-5 text-center"><p className="text-[10px] uppercase tracking-[0.28em] text-[#8b5e3c]">Votre réponse</p><h2 className="mt-2 font-serif text-2xl text-[#536755]">Serez-vous des nôtres ?</h2></div>
    ...
    <div className="mx-auto max-w-3xl">
      ...
      {guests === null ? (
        <form onSubmit={login} className="mx-auto max-w-sm rounded-3xl border border-[#d8ccb9] bg-[#fffaf1] p-8 text-center">
          <LockKeyhole size={22} className="mx-auto mb-4 text-[#8b5e3c]" />
        ...
        <>
          <div className="mb-9 flex items-end justify-between">
            <div><p className="text-[10px] uppercase tracking-[0.28em] text-[#8b5e3c]">Espace privé</p><h1 className="mt-2 font-serif text-4xl">Les invités</h1></div>
...
export default function Page() {
  ...
  if (splash) return <main className="splash-screen"><div className={`text-center transition duration-700 ${opened ? "scale-110 opacity-0" : ""}`}><div className="envelope-wrap mx-auto mb-7 cursor-pointer" onClick={openInvitation} role="button" tabIndex={0} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") openInvitation() }} aria-label="Ouvrir l'invitation"><div className="envelope"><div className="envelope-flap" /><div className="envelope-seal"><Gift size={17} /></div></div></div><p className="font-serif text-2xl italic text-[#536755] sm:text-3xl">Prêts à célébrer<br />un moment magique ?</p><p className="mt-4 text-[10px] uppercase tracking-[0.2em] text-[#8b5e3c]">Cliquez pour ouvrir</p></div>{opened && <Confetti />}</main>
  if (admin) return <Admin onBack={() => setAdmin(false)} />
  return <main className="min-h-screen overflow-hidden bg-[#f5eddf] px-4 py-8 text-[#536755] sm:px-6"><div className="mx-auto max-w-xl"><div className="mb-7 flex items-center justify-between px-2"><span className="text-[10px] uppercase tracking-[0.28em] text-[#9a927f]">Invitation privée</span><span className="flex items-center gap-1.5 text-[10px] uppercase tracking-[0.18em] text-[#8b5e3c]"><CalendarDays size={13} /> 21.11.26</span></div><section className="ribbon-frame"><div className="ribbon-inner relative rounded-[2rem] border border-[#8b5e3c]/45 bg-[#fffaf1] px-6 py-10 text-center shadow-[0_10px_30px_rgba(91,75,48,0.08)] sm:px-12"><div className="pointer-events-none absolute inset-3 rounded-[1.6rem] border border-[#8b5e3c]/20" /><div className="relative"><div className="mx-auto mb-7 flex h-11 w-11 items-center justify-center rounded-full bg-[#dce5d7] text-[#788b76]"><Gift size={20} strokeWidth={1.4} /></div><p className="text-[10px] uppercase tracking-[0.4em] text-[#8b5e3c]">Une première bougie, un grand bonheur à partager...</p><h1 className="mt-4 font-serif text-6xl italic tracking-tight text-[#536755] sm:text-7xl">Eyalane</h1><div className="mx-auto my-5 h-28 w-28 overflow-hidden rounded-full border-[3px] border-[#8b5e3c]/70 shadow-[0_8px_20px_rgba(91,75,48,0.18)]"><img src="/photos/eyalane.jpg" alt="Eyalane" className="h-full w-full object-cover" /></div><p className="font-serif text-3xl text-[#8b5e3c]">1 an</p><p className="mx-auto mt-5 max-w-xs text-sm leading-6 text-[#7d8273]">venez célébrer ce merveilleux moment à nos côtés et créer avec nous de précieux souvenirs ☺️</p><div className="mt-7 flex items-start justify-center divide-x divide-[#8b5e3c]/30 text-[10px] text-[#687166]"><div className="flex min-w-0 flex-1 flex-col items-center gap-2 px-2"><CalendarDays size={15} className="text-[#8b5e3c]" /><span>21 novembre</span></div><div className="flex min-w-0 flex-1 flex-col items-center gap-2 px-2"><Clock3 size={15} className="text-[#8b5e3c]" /><span>13h00</span></div><div className="flex min-w-0 flex-1 flex-col items-center gap-2 px-2"><MapPin size={15} className="text-[#8b5e3c]" /><span>Viry Châtillon</span></div></div><p className="mx-auto mt-6 w-fit rounded-full border border-[#8b5e3c]/25 bg-[#fbf6eb] px-4 py-2 text-[10px] tracking-wide text-[#78806f]">Code couleur : Vert sauge, beige &amp; marron</p><Countdown /><RsvpForm /></div></div></section><button onClick={() => setAdmin(true)} aria-label="Ouvrir la vue admin" className="fixed bottom-4 right-4 rounded-full border border-[#8b5e3c]/30 bg-[#fffaf1]/80 p-3 text-[#9b8764] shadow-sm backdrop-blur transition hover:bg-white"><LockKeyhole size={14} /></button><p className="py-7 text-center text-[10px] uppercase tracking-[0.28em] text-[#aaa18f]">Avec tout notre amour</p></div></main>
}
