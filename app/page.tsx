...
export default function Page() {
  ...
  return <main className="min-h-screen overflow-hidden bg-[#f5eddf] px-4 py-8 text-[#536755] sm:px-6"><div className="mx-auto max-w-xl"><div className="mb-7 flex items-center justify-between px-2"><span className="text-[10px] uppercase tracking-[0.28em] text-[#9a927f]">Invitation privée</span><span className="flex items-center gap-1.5 text-[10px] uppercase tracking-[0.18em] text-[#536755]"><CalendarDays size={13} /> 21.11.26</span></div><section className="relative overflow-hidden rounded-[2rem] border border-[#536755]/45 bg-[#fffaf1] px-6 py-10 text-center shadow-[0_18px_55px_rgba(83,103,85,0.14)] sm:px-12"><span className="corner-ribbon corner-ribbon-left" aria-hidden="true" /><span className="corner-ribbon corner-ribbon-right" aria-hidden="true" /><div className="pointer-events-none absolute inset-3 rounded-[1.6rem] border border-[#536755]/20" /><div aria-hidden="true" className="pointer-events-none absolute inset-0 z-0 select-none text-2xl">
      <span className="absolute left-2 top-2 -rotate-12 opacity-50">🦁</span>
      <span className="absolute right-2 top-1 rotate-6 opacity-50">🦒</span>
      <span className="absolute left-1/4 top-1 rotate-3 opacity-40 text-xl">🦋</span>
      <span className="absolute right-1/4 top-2 -rotate-6 opacity-40 text-xl">🐦</span>
      <span className="absolute left-3 top-16 -rotate-12 opacity-45">🐘</span>
      <span className="absolute right-4 top-14 rotate-6 opacity-45">🦓</span>
      <span className="absolute left-2 top-44 -rotate-6 opacity-35 text-xl">🐒</span>
      <span className="absolute right-2 top-44 rotate-12 opacity-35 text-xl">🐆</span>
      <span className="absolute left-3 top-60 rotate-6 opacity-30 text-lg">🌴</span>
      <span className="absolute right-3 top-60 -rotate-6 opacity-30 text-lg">🦜</span>
      <span className="absolute left-2 top-72 -rotate-3 opacity-30 text-lg">🐾</span>
      <span className="absolute right-2 top-72 rotate-3 opacity-30 text-lg">☀️</span>
      <span className="absolute left-3 bottom-2 rotate-6 opacity-45">🌿</span>
      <span className="absolute right-3 bottom-1 -rotate-6 opacity-45">🌾</span>
      <span className="absolute left-1/4 bottom-2 -rotate-3 opacity-35 text-xl">🦋</span>
      <span className="absolute right-1/4 bottom-1 rotate-3 opacity-35 text-xl">🐦</span>
      <span className="absolute left-1/2 bottom-6 -translate-x-1/2 rotate-3 opacity-30 text-lg">🐘</span>
