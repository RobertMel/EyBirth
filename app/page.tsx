...
export default function Page() {
  ...
  return <main className="min-h-screen overflow-hidden bg-[#f5eddf] px-4 py-8 text-[#536755] sm:px-6">
    <div className="mx-auto max-w-xl">
      <div className="mb-7 flex items-center justify-between px-2"><span className="text-[10px] uppercase tracking-[0.28em] text-[#9a927f]">Invitation privée</span><span className="flex items-center gap-1.5 text-[10px] uppercase tracking-[0.18em] text-[#536755]"><CalendarDays size={13} /> 21.11.26</span></div>

      <section className="invite-card relative overflow-hidden rounded-[1.4rem] shadow-[0_18px_55px_rgba(83,103,85,0.16)]">
        <img src="/frame/invitation-frame.jpg" alt="" aria-hidden="true" className="pointer-events-none absolute inset-0 h-full w-full select-none" />
        <div className="invite-content relative z-10 flex flex-col items-center text-center">
          ...
          <div className="inv-info flex items-start justify-center divide-x divide-[#536755]/30 text-[#687166]">
            <div className="flex flex-1 flex-col items-center"><CalendarDays className="inv-info-icon" /><span>21 novembre</span></div>
            <div className="flex flex-1 flex-col items-center"><Clock3 className="inv-info-icon" /><span>13h00</span></div>
            <div className="flex flex-1 flex-col items-center"><MapPin className="inv-info-icon" /><span>Viry-Châtillon</span></div>
          </div>
          <div className="inv-dress flex items-center rounded-full border border-[#536755]/25 bg-[#fbf6eb]/90 text-[#78806f]"><span className="font-medium text-[#536755]">Dress code</span><span className="flex items-center"><span className="inv-dot rounded-full border border-[#4a5a4c]/20 bg-[#536755]" />Vert sauge</span><span className="flex items-center"><span className="inv-dot rounded-full border border-[#d9cdb8] bg-[#ece2cf]" />Beige</span></div>
        </div>
        <p className="inv-signature z-10 text-center font-serif italic text-[#536755]">Avec tout notre amour</p>
      </section>

      <section className="mt-6 rounded-[1.4rem] border border-[#536755]/25 bg-[#fffaf1] px-6 pb-10 pt-2 text-center shadow-[0_18px_55px_rgba(83,103,85,0.12)] sm:px-12">
        <Countdown />
        <RsvpForm />
      </section>

      <button onClick={() => setAdmin(true)} aria-label="Ouvrir la vue admin" className="fixed bottom-4 right-4 rounded-full border border-[#536755]/30 bg-[#fffaf1]/80 p-3 text-[#9b8764] shadow-sm backdrop-blur transition hover:bg-white"><LockKeyhole size={14} /></button>
      <div className="h-8" />
    </div>
  </main>
}
